...
<xsl:template name="summary">
	...
	<table class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
		...
		<tr valign="top">
			...
			<xsl:variable name="allFailureCount" select="count(/testResults/*[attribute::success='false'])" />
			<xsl:variable name="allSuccessCount" select="count(/testResults/*[attribute::success='true'])" />
			<xsl:variable name="allSuccessPercent" select="$allSuccessCount div $allCount" />
			<xsl:variable name="allTotalTime" select="sum(/testResults/*/@time)" />
			<xsl:variable name="allAverageTime" select="$allTotalTime div $allCount" />
			<xsl:variable name="allMinTime">
				<xsl:call-template name="min">
					<xsl:with-param name="nodes" select="/testResults/*/@time" />
				</xsl:call-template>
			</xsl:variable>
...
<xsl:template name="pagelist">
	...
	<table class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
		...
		<xsl:for-each select="/testResults/*[not(@label = preceding::*/@label)]">
			...
			<xsl:variable name="count" select="count(../*[@label = current()/@label])" />
			<xsl:variable name="failureCount" select="count(../*[@label = current()/@label][attribute::success='false'])" />
			<xsl:variable name="successCount" select="count(../*[@label = current()/@label][attribute::success='true'])" />
			<xsl:variable name="successPercent" select="$successCount div $count" />
			<xsl:variable name="totalTime" select="sum(../*[@label = current()/@label]/@time)" />
			<xsl:variable name="averageTime" select="$totalTime div $count" />
			<xsl:variable name="minTime">
				<xsl:call-template name="min">
					<xsl:with-param name="nodes" select="../*[@label = current()/@label]/@time" />
				</xsl:call-template>
			</xsl:variable>
			...
			<tr valign="top">
				...
				<td align="center">
				   <a href="">
			         ...
			         <xsl:for-each select="../*[@label = $label and @threadName != $label]">			         			            
			            <tr>
	...
	<xsl:if test="$allFailureCount > 0">
		...
		<xsl:for-each select="/testResults/*[not(@label = preceding::*/@label)]">
			...
			<xsl:if test="$failureCount > 0">
				...
				<xsl:for-each select="/testResults/*[@label = current()/@label][attribute::success='false']">
					<tr>
